  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-bgCoal p-8 flex flex-col items-center justify-center">
        ...
        <p className="mt-4 text-gray-400 text-sm animate-pulse">Cargando perfiles...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-bgCoal p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-light text-white mb-1">Perfiles de Leads</h1>
            <p className="text-sm text-gray-400">Gestiona y monitorea tus leads</p>
          </div>
          <div className="relative">
            <input
              ...
              className="w-64 bg-gray-900/50 text-white text-sm border border-gray-800 rounded-full px-4 py-2 pl-10 focus:outline-none focus:border-gray-700 focus:ring-1 focus:ring-gray-700"
            />
            ...
            <thead>
              <tr className="border-b border-gray-700/50">
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400">
                  <button onClick={() => handleSort('name_client')} className="flex items-center gap-2 hover:text-white">
                    Nombre del Cliente
                    <ChevronUpDownIcon className="w-3 h-3" />
                  </button>
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400">
                  <button onClick={() => handleSort('name_company')} className="flex items-center gap-2 hover:text-white">
                    Empresa
                    <ChevronUpDownIcon className="w-3 h-3" />
                  </button>
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400">Redes Sociales</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400">
                ...
                leads.map((lead) => (
                  <tr key={lead.id} className="hover:bg-gray-700/30">
                    <td className="px-4 py-4">
                      <div className="text-sm text-white">{lead.name_client}</div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="text-sm text-white">{lead.name_company}</div>
                      <div className="text-xs text-gray-400 mt-1 line-clamp-2">{lead.description_company}</div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex space-x-2">
                        {lead.instagram && (
                          <a href={lead.instagram} target="_blank" rel="noopener noreferrer" 
                             className="text-gray-400 hover:text-prinFuchsia">
                    ...
                    <td className="px-4 py-4">
                      <button
                        ...
                        className="text-gray-400 hover:text-prinFuchsia transition-colors"
                      >
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <div className="bg-gray-900/90 w-full max-w-md rounded-2xl border border-gray-800 shadow-xl">
              <div className="p-6">
                <div className="mb-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-gray-800/50 text-white border border-gray-700/50 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-prinFuchsia"
                      required
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-gray-800/50 text-white border border-gray-700/50 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-prinFuchsia resize-none"
                      rows={4}
                      required
                    />
                  </div>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-sm font-medium text-gray-300">Redes Sociales</h3>
                    <div>
                      <label className="block text-sm text-gray-400 mb-1">
                        Instagram
                      </label>
                  ...
                  <div className="flex justify-end gap-3 pt-4">
                    ...
                    <button
                      type="submit"
                      className="px-4 py-2 text-sm bg-prinFuchsia text-white rounded-lg hover:bg-opacity-90 transition-colors"
                    >
                      Guardar cambios
                    </button>
                  </div>
